  ...
  <head>
    ...
    <script language="javascript">
        ...
        function deleteBook(id){
        	...
        	if(b){
        		window.location.href="${pageContext.request.contextPath}/admin/BookServlet?method=deleteBook&jspName=all&delid=" + id;	
        	}
        }
    </script>
</head>
...
<div class="wrap">
    ...
    <div class="content">
        ...
        <div class="sa2book">
        <table border="1">
        	<c:if test="${empty allBook.beanMap}">
        		数据库无图书记录！
        	</c:if>
        	<c:if test="${!(empty allBook.beanMap)}">
            <tr>
            ...
            <c:forEach var="book" items="${allBook.beanMap}">
            <tr>
                ...
                <td><a href="<c:url value='/admin/AdminUI?jspPage=updateBookFromAll&upid=${book.key}'/>">[修改]</a></td>
        ...
        <div class="fixdown">
            <center>
            <c:choose>
            	<c:when test="${allBook.tp eq 0}">
            		
            	</c:when>
            	<c:otherwise>
            第${allBook.pc}页/共${allBook.tp}页&nbsp;
            <a href="<c:url value='/admin/BookServlet?method=showAllBook&pc=1'/>">首页</a>
            <c:if test="${allBook.pc > 1}">
            	<a href="<c:url value='/admin/BookServlet?method=showAllBook&pc=${allBook.pc-1}'/>">上一页</a>
            </c:if>
            <c:if test="${allBook.pc < allBook.tp}">
            	<a href="<c:url value='/admin/BookServlet?method=showAllBook&pc=${allBook.pc+1}'/>">下一页</a>
            </c:if>
            <a href="<c:url value='/admin/BookServlet?method=showAllBook&pc=${allBook.tp}'/>">尾页</a>
            </c:otherwise>
            </c:choose>
            </center>
        </div>
