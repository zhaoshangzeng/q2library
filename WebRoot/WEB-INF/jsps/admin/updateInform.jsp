...
<div class="wrap">
    ...
    <div class="content">
        ...
        <div class="a_inform">
        <form action="<c:url value='/admin/InformServlet'/>" method="post">
        	<input type="hidden" name="method" value="updateInform"/>
            <input type="hidden" name="iid" value="${upInform.iid}"/>
            <input type="hidden" name="itime" value="${upInform.itime}"/>
            <label>通知的标题：<input type="text" name="title" value="${upInform.title}"></label><br/>
            <div>通知内容:</div>
            <label><textarea name="content" id="" cols="100" rows="10">${upInform.content}</textarea></label><br/>
            <input type="reset"><input type="submit" value="提交">
            <p><span style="color:red;font-size:30px;">${upInformMeg}</span></p>
        </form>
    	</div>
    </div>
</div>
